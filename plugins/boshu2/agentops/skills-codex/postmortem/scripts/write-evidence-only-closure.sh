...
if [[ -z "$WORKSPACE_ROOT" ]]; then
  echo "postmortem: cannot locate workspace assets (schemas/evidence-only-closure.v1.schema.json, lib/hook-helpers.sh)" >&2
...
usage() {
  cat <<'EOF'
Usage: bash skills/postmortem/scripts/write-evidence-only-closure.sh [options]
...
Options:
  ...
  --producer <name>             Producer name recorded in the artifact. Default: postmortem.
