# Goals Operations Reference

Detailed mode-by-mode procedures retained outside the execution kernel. The
canonical routing, constraints, output contract, and quality bar live in
`../SKILL.md`; use this reference only after selecting a mode there.
...
contracts are documented in `executable-spec-chain.md`.
...
Product directives sit alongside engineering ones in the same `## Directives` section. See `generation-heuristics.md` for product directive patterns.
...
human-gate semantics: `executable-spec-chain.md` and
`docs/adr/ADR-0006`.
...
Outputs the snapshot to stdout in the fitness snapshot schema (see `goals-schema.md`).
...
See `goals-schema.md` for the meta-goal pattern.
...
- `goals-schema.md` — schema definition for both formats
- `generation-heuristics.md` — goal quality criteria

## Reference Documents

- [goals.feature](goals.feature) — Executable spec: measure gates → verdict, directives steering, GOALS.md source-of-truth, measurement-root (soc-qk4b)
- [generation-heuristics.md](generation-heuristics.md)
- [goals-schema.md](goals-schema.md)
- [executable-spec-chain.md](executable-spec-chain.md)
